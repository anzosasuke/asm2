...
def add_instr_to_node(angr_node, asm2vec_node):
    if angr_node.block is not None:
        # print(angr_node.block.capstone.insns)
...
def create_asm2vec_function(entry_node, cfg, func_name):
    ...
    # print(func_name, "function created" + "\n")
...
def create_function_cfg(binary, function):
    ...
    if function_symbol is not None:
        ...
        entry_node = cfg.model.get_any_node(function_symbol.rebased_addr)
        #print(entry_node)
...
def function_asm(binary):
    ...
    funcs = []
    for sym in symbols:
        if "." not in str(sym) and "_" not in str(sym):
            function_symbol = p.loader.find_symbol(sym)
        # print(function_symbol)
            if function_symbol is not None:
                cfg = p.analyses.CFGEmulated(starts=(function_symbol.rebased_addr,), call_depth=1, normalize=True)
                entry_node = cfg.model.get_any_node(function_symbol.rebased_addr)
                # print(entry_node)
                funcs.append([cfg, entry_node, sym])
    return funcs
